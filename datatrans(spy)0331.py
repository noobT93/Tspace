...
writer = pd.ExcelWriter(r'C:/Users/Tao/Desktop/投资/spy/0331/日转数据.xlsx')
for i in ['spy','vix','dji','ixic','spx','hsi','sh','sz','cy']:
    locals()[i+'df'] = pd.read_excel(r'C:/Users/Tao/Desktop/投资/spy/0331/日数据.xlsx',sheet_name=[
    ...
    locals()[i + 'df2']['Open'] = locals()[i + 'df']['Open'].resample('W', label='right').last()
    ...
    if i not in ['hsi', 'sh','sz','cy']:
        ...
        if i != 'vix':
            ...
            for dnum in range(0, len(locals()[i + 'df1'].index), 1):
                ...
                tlist = [datei for datei in locals()[i + 'df'].index.values if aaa <= pd.to_datetime(datei) < bbb]
                highd = \
                    locals()[i + 'df'].loc[tlist, :][
                        locals()[i + 'df'].loc[tlist, :]['High'] == locals()[i + 'df1'].iloc[
                            dnum, 1]].index.values[0]
                locals()[i + 'df1'].iloc[dnum, -2] = locals()[i + 'df'].loc[highd, 'Volume']
                lowd = \
                    locals()[i + 'df'].loc[tlist, :][
                        locals()[i + 'df'].loc[tlist, :]['Low'] == locals()[i + 'df1'].iloc[
...
writer1 = pd.ExcelWriter(r'C:/Users/Tao/Desktop/投资/spy/0331/日转周数据.xlsx')
for i in ['spy','vix','dji','ixic','spx','hsi','sh','sz','cy']:
    locals()[i+'df'] = pd.read_excel(r'C:/Users/Tao/Desktop/投资/spy/0331/日数据.xlsx',sheet_name=[
        'spy','vix','dji','ixic','spx','hsi','sh','sz','cy'].index(i))
    locals()[i + 'df']['Date'] = locals()[i + 'df']['Date'].map(lambda y:pd.to_datetime(y) )
