...
#主表
final_df = pd.read_excel(r'C:/Users/Tao/Desktop/投资/spy/vol下单模板0630.xlsx',sheet_name = 1)
...
final_df.iloc[7, 13] = range_df[range_df.收盘== range_df.收盘[-1]]['总手(万)'].values

#上证T-1周
try:
    final_df.iloc[1,13] = sh_df1[sh_df1.index == (end - dt.timedelta(days=6))].最高.values
    final_df.iloc[3,13] = sh_df1[sh_df1.index == (end - dt.timedelta(days=6))].最低.values
except:
    final_df.iloc[1,13] = sh_df1[sh_df1.index == (end - dt.timedelta(days=13))].最高.values
    final_df.iloc[3,13] = sh_df1[sh_df1.index == (end - dt.timedelta(days=13))].最低.values'''
...
#final_df.iloc[9, 13] = range_df[range_df.最低== min(range_df.最低)]['总手(万)'].values
...
final_df.iloc[3, 17] = range_df[range_df.最高== max(range_df.最高)]['总手(万)'].values[0]
final_df.iloc[4, 17] = range_df[range_df.最低== min(range_df.最低)]['总手(万)'].values[0]
...
final_df.iloc[0, 17] = range_df[range_df.收盘== range_df.收盘[-1]]['总手(万)'].values[-1]
...
final_df.iloc[1, 17] = range_df[range_df.收盘== range_df.收盘[-1]]['总手(万)'].values[-1]
...
final_df.iloc[2, 17] = range_df[range_df.收盘== range_df.收盘[-1]]['总手(万)'].values[-1]
...
final_df.iloc[5,1] =spy[spy.High == high_value].Volume.values'''
...
final_df.iloc[3,7] =ixic[ixic.High == high_value].Volume.values
...
final_df.iloc[2,9] =spx.Open.resample('W', label='right').last().values
final_df.iloc[6,9] =spx.High.resample('W', label='right').max().values
final_df.iloc[7,9] =spx.Low.resample('W', label='right').min().values
high_value = spx.High.resample('W', label='right').max().values[0]
final_df.iloc[8,9] =spx[spx.High == high_value].Volume.values[0]
low_value = spx.Low.resample('W', label='right').min().values[0]
final_df.iloc[9,9] =spx[spx.Low == low_value].Volume.values[0]
...
final_df.iloc[6,5] =dji[dji.Low == low_value].Volume.values'''

ixic= web.DataReader('^IXIC', 'yahoo',end - dt.timedelta(days=20),end - dt.timedelta(days=14))
final_df.iloc[0,7] =ixic.Open.resample('W', label='right').last().values
'''high_value = ixic.High.resample('W', label='right').max().values[0]
...
final_df.iloc[4,9] =spx.Open.resample('W', label='right').last().values
'''low_value = spx.Low.resample('W', label='right').min().values[0]
final_df.iloc[8,9] =spx[spx.Low == low_value].Volume.values'''
final_df.iloc[10,9] =spx.Volume.resample('W', label='right').last().values
...
final_df.iloc[1,7] =ixic.Close.resample('W', label='right').last().values
'''high_value = ixic.High.resample('W', label='right').max().values[0]
...
final_df.iloc[9,9] =spx.Volume.resample('W', label='right').last().values'''
...
final_df.to_excel(r'C:/Users/Tao/Desktop/投资/spy/vol取数.xlsx' , index=True, encoding='utf_8_sig')