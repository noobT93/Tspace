...
if 'const' not in backward_regression(data, y_value, 0.05, verbose=False)[-1]:
    ...
    vixlast_df['adjfore'] = [math.exp(i) for i in backward_regression(
        data, y_value, 0.05, verbose=False)[0].predict(vixlast_df[testvariable])]
else:
    testvariable = backward_regression(data, y_value, 0.05, verbose=False)[-1]
    testresult1 = [math.exp(i) for i in backward_regression(
        data, y_value, 0.05, verbose=False)[0].predict(sm.add_constant(vixlast_df.iloc[-1:,:])[testvariable])]
    vixlast_df['adjfore'] = [math.exp(i) for i in backward_regression(
        data, y_value, 0.05, verbose=False)[0].predict(sm.add_constant(vixlast_df)[testvariable])]

vixlast_df['adjdiff'] = 1
# 生成adjratio
for i in vixlast_df.index:
    if (math.exp(vixlast_df.loc[i, vixlast_df.columns[0]]) > 1.2 and vixlast_df.loc[i, 'adjfore'] < 1.2) or (
            math.exp(vixlast_df.loc[i, vixlast_df.columns[0]]) < 1.2 and vixlast_df.loc[i, 'adjfore'] > 1.2):
        vixlast_df.loc[i, 'adjdiff'] = math.exp(vixlast_df.loc[i, vixlast_df.columns[0]]) / vixlast_df.loc[
            i, 'adjfore']

adjratio = st.gmean(vixlast_df.adjdiff)

updateresult = pd.DataFrame(index =['spy','vix（反向）'],columns = ['结果','判断'])
updateresult.loc['spy','结果'] = testresult[0]
updateresult.loc['vix（反向）','结果'] = testresult1[0] *adjratio
...
updateresult.to_excel((r'C:/Users/Tao/Desktop/投资/final/更新结果adj.xlsx'))
