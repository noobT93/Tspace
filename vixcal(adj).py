...
for data_freq in [1,3]:
    ...
    for datenum in date_init:
        ...
        if 'const' not in backward_regression(data, y_value, 0.05, verbose=False)[-1]:
            ...
            # 生成preforereulst
            traindata_df['adjfore'] = [math.exp(i) for i in backward_regression(
        ...
        for i in traindata_df.index:
            if (math.exp(traindata_df.loc[i, traindata_df.columns[0]]) > 1.2 and traindata_df.loc[i, 'adjfore'] < 1.2) or (
                    math.exp(traindata_df.loc[i, traindata_df.columns[0]]) < 1.2 and traindata_df.loc[i, 'adjfore'] > 1.2):
                traindata_df.loc[i, 'adjdiff'] = math.exp(traindata_df.loc[i, traindata_df.columns[0]]) / traindata_df.loc[
                    i, 'adjfore']
        ...
        for i in testdata_df.index:
            if (locals()['width3freq' + str(data_freq)].loc[i, 'foreresult'] >= 1.2 and
                locals()['width3freq' + str(data_freq)].loc[i, 'vixmaxcoe'] >= 1.2) or (
                    locals()['width3freq' + str(data_freq)].loc[i, 'foreresult'] <= 1.2 and
                    locals()['width3freq' + str(data_freq)].loc[i, 'vixmaxcoe'] <= 1.2):
            ...
            if (locals()['width3freq' + str(data_freq)].loc[i, 'adjresult'] >= 1.2 and
                locals()['width3freq' + str(data_freq)].loc[i, 'vixmaxcoe'] >= 1.2) or (
                    locals()['width3freq' + str(data_freq)].loc[i, 'adjresult'] <= 1.2 and
                    locals()['width3freq' + str(data_freq)].loc[i, 'vixmaxcoe'] <= 1.2):
                locals()['width3freq' + str(data_freq)].loc[i, 'adjjudge'] = 1

    print (locals()['width3freq' + str(data_freq)])
    finaladjresult = locals()['width3freq' + str(data_freq)].adjjudge.sum() / len(
        locals()['width3freq' + str(data_freq)].index)
    traintest_df['width3freq' + str(data_freq)] = finaladjresult

print (traintest_df)
traintest_df.to_excel((r'C:/Users/Tao/Desktop/投资/final/vix回测结果adj.xlsx'))
