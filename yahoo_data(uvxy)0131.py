...
#截止时间（当周下单日）
end = dt.datetime(2020,2,28)

#主表
final_df = pd.read_excel(r'C:/Users/Tao/Desktop/投资/uvxy/下单模板(p_vol)0131.xlsx',sheet_name = 1)
...
final_df.iloc[4,3] =uvxy['Volume'].resample('W', label='right').last().values
...
final_df.iloc[7,5] =dji['Volume'].resample('W', label='right').last().values
...
final_df.iloc[3,1] =vix['Close'].resample('W', label='right').last().values
...
final_df.iloc[5,3] =uvxy[uvxy.High == high_value].Volume.values
final_df.iloc[6,3] =uvxy['Volume'].resample('W', label='right').last().values
...
final_df.iloc[5,7] =ixic['Close'].resample('W', label='right').last().values
...
final_df.iloc[1,7] =ixic['High'].resample('W', label='right').max().values
