...
#截止时间（当周下单日）
end = dt.datetime(2020,6,13)
#检查时把取数中A股隐藏避免取数错误

#主表
final_df = pd.read_excel(r'C:/Users/Tao/Desktop/投资/spy/stoploss下单模板0731.xlsx',sheet_name = 1)
...
#上证T周
try:
    final_df.iloc[0,13] = sh_df1[sh_df1.index == (end + dt.timedelta(days=1))].收盘.values
    range_df = sh_df.loc[[datei for datei in sh_df.index if sh_df1.index[-2] < datei < sh_df1.index[-1]], :]
except:
    final_df.iloc[0,13] = sh_df1[sh_df1.index == (end - dt.timedelta(days=6))].收盘.values
...
#上证T-1周
try:
    final_df.iloc[1,13] = sh_df1[sh_df1.index == (end - dt.timedelta(days=6))].收盘.values
    range_df = sh_df.loc[[datei for datei in sh_df.index if sh_df1.index[-3] < datei < sh_df1.index[-2]], :]
except:
    final_df.iloc[1,13] = sh_df1[sh_df1.index == (end - dt.timedelta(days=13))].收盘.values
    range_df = sh_df.loc[[datei for datei in sh_df.index if sh_df1.index[-4] < datei < sh_df1.index[-2]], :]

#final_df.iloc[2, 13] = range_df[range_df.收盘== range_df.收盘[-1]]['总手(万)'].values[-1]

#上证T-2周
try:
    final_df.iloc[2,13] = sh_df1[sh_df1.index == (end - dt.timedelta(days=13))].收盘.values
    final_df.iloc[4, 13] = sh_df1[sh_df1.index == (end - dt.timedelta(days=13))].最高.values
    final_df.iloc[5, 13] = sh_df1[sh_df1.index == (end - dt.timedelta(days=13))].最低.values
    range_df = sh_df.loc[[datei for datei in sh_df.index if sh_df1.index[-4] < datei < sh_df1.index[-3]], :]
except:
    final_df.iloc[2,13] = sh_df1[sh_df1.index == (end - dt.timedelta(days=20))].收盘.values
    final_df.iloc[4, 13] = sh_df1[sh_df1.index == (end - dt.timedelta(days=20))].最高.values
...
final_df.iloc[6, 13] = range_df[range_df.最高== max(range_df.最高)]['总手(万)'].values[0]
final_df.iloc[7, 13] = range_df[range_df.最低== min(range_df.最低)]['总手(万)'].values[0]


#上证T-3周
try:
    final_df.iloc[3, 13] = sh_df1[sh_df1.index == (end - dt.timedelta(days=20))].收盘.values
    #final_df.iloc[5, 13] = sh_df1[sh_df1.index == (end - dt.timedelta(days=20))].最低.values
    range_df = sh_df.loc[[datei for datei in sh_df.index if sh_df1.index[-5] < datei < sh_df1.index[-4]], :]
except:
    final_df.iloc[3, 13] = sh_df1[sh_df1.index == (end - dt.timedelta(days=27))].收盘.values
...
final_df.iloc[8, 13] = range_df[range_df.收盘== range_df.收盘[-1]]['总手(万)'].values
...
#深证T周
try:
    final_df.iloc[3,15] = sz_df1[sz_df1.index == (end + dt.timedelta(days=1))].最高.values
    final_df.iloc[4,15] = sz_df1[sz_df1.index == (end + dt.timedelta(days=1))].最低.values
    range_df = sz_df.loc[[datei for datei in sz_df.index if sz_df1.index[-2] < datei < sz_df1.index[-1]], :]
except:
    final_df.iloc[3,15] = sz_df1[sz_df1.index == (end - dt.timedelta(days=6))].最高.values
    final_df.iloc[4,15] = sz_df1[sz_df1.index == (end - dt.timedelta(days=6))].最低.values
    range_df = sz_df.loc[[datei for datei in sz_df.index if sz_df1.index[-3] < datei < sz_df1.index[-1]], :]

final_df.iloc[7, 15] = range_df[range_df.收盘== range_df.收盘[-1]]['总手(万)'].values
...
#深证T-1周
try:
    final_df.iloc[0,15] = sz_df1[sz_df1.index == (end - dt.timedelta(days=6))].收盘.values
    range_df = sz_df.loc[[datei for datei in sz_df.index if sz_df1.index[-3] < datei < sz_df1.index[-2]], :]
except:
    final_df.iloc[0,15] = sz_df1[sz_df1.index == (end - dt.timedelta(days=13))].收盘.values
    range_df = sz_df.loc[[datei for datei in sz_df.index if sz_df1.index[-4] < datei < sz_df1.index[-2]], :]

final_df.iloc[10, 15] = range_df[range_df.收盘== range_df.收盘[-1]]['总手(万)'].values[-1]

#深证T-2周
try:
    final_df.iloc[1,15] = sz_df1[sz_df1.index == (end - dt.timedelta(days=13))].收盘.values
    final_df.iloc[5,15] = sz_df1[sz_df1.index == (end - dt.timedelta(days=13))].最高.values
    final_df.iloc[6,15] = sz_df1[sz_df1.index == (end - dt.timedelta(days=13))].最低.values
    range_df = sz_df.loc[[datei for datei in sz_df.index if sz_df1.index[-4] < datei < sz_df1.index[-3]], :]
except:
    final_df.iloc[1,15] = sz_df1[sz_df1.index == (end - dt.timedelta(days=20))].收盘.values
    final_df.iloc[5,15] = sz_df1[sz_df1.index == (end - dt.timedelta(days=20))].最高.values
    final_df.iloc[6,15] = sz_df1[sz_df1.index == (end - dt.timedelta(days=20))].最低.values
...
#创业T周
try:
    final_df.iloc[0,17] = cy_df1[cy_df1.index == (end + dt.timedelta(days=1))].最高.values
    final_df.iloc[1,17] = cy_df1[cy_df1.index == (end + dt.timedelta(days=1))].最低.values
    range_df = cy_df.loc[[datei for datei in cy_df.index if cy_df1.index[-2] < datei < cy_df1.index[-1]], :]
except:
    final_df.iloc[0,17] = cy_df1[cy_df1.index == (end - dt.timedelta(days=6))].最高.values
    final_df.iloc[1,17] = cy_df1[cy_df1.index == (end - dt.timedelta(days=6))].最低.values
...
final_df.iloc[5,17] = range_df[range_df.最高== max(range_df.最高)]['总手(万)'].values[0]
final_df.iloc[6,17] = range_df[range_df.最低== min(range_df.最低)]['总手(万)'].values[0]

#创业T-1周
try:
    final_df.iloc[2,17] = cy_df1[cy_df1.index == (end - dt.timedelta(days=6))].收盘.values
    range_df = cy_df.loc[[datei for datei in cy_df.index if cy_df1.index[-3] < datei < cy_df1.index[-2]], :]
except:
    final_df.iloc[2,17] = cy_df1[cy_df1.index == (end - dt.timedelta(days=13))].收盘.values
...
#创业T-2周
try:
    final_df.iloc[7,17] = cy_df1[cy_df1.index == (end - dt.timedelta(days=13))].最高.values
    final_df.iloc[8,17] = cy_df1[cy_df1.index == (end - dt.timedelta(days=13))].最低.values
    range_df = cy_df.loc[[datei for datei in cy_df.index if cy_df1.index[-4] < datei < cy_df1.index[-3]], :]
except:
    final_df.iloc[7,17] = cy_df1[cy_df1.index == (end - dt.timedelta(days=20))].最高.values
    final_df.iloc[8,17] = cy_df1[cy_df1.index == (end - dt.timedelta(days=20))].最低.values
...
#创业T-3周
try:
    final_df.iloc[9, 17] = cy_df1[cy_df1.index == (end - dt.timedelta(days=20))].收盘.values
    range_df = cy_df.loc[[datei for datei in cy_df.index if cy_df1.index[-5] < datei < cy_df1.index[-4]], :]
except:
    final_df.iloc[9, 17] = cy_df1[cy_df1.index == (end - dt.timedelta(days=27))].收盘.values
...
final_df.iloc[7,5] =dji.High.resample('W', label='right').max().values
final_df.iloc[8,5] =dji.Low.resample('W', label='right').min().values
high_value = dji.High.resample('W', label='right').max().values[0]
final_df.iloc[9,5] =dji[dji.High == high_value].Volume.values[0]
low_value = dji.Low.resample('W', label='right').min().values[0]
final_df.iloc[10,5] =dji[dji.Low == low_value].Volume.values[0]

ixic= web.DataReader('^IXIC', 'yahoo',end - dt.timedelta(days=6),end )
final_df.iloc[0,7] =ixic.Volume.resample('W', label='right').last().values
final_df.iloc[4,7] =ixic.High.resample('W', label='right').max().values
final_df.iloc[5,7] =ixic.Low.resample('W', label='right').min().values
high_value = ixic.High.resample('W', label='right').max().values[0]
final_df.iloc[7,7] =ixic[ixic.High == high_value].Volume.values[0]
low_value = ixic.Low.resample('W', label='right').min().values[0]
final_df.iloc[8,7] =ixic[ixic.Low == low_value].Volume.values[0]

spx= web.DataReader('^GSPC', 'yahoo',end - dt.timedelta(days=6),end )
final_df.iloc[0,9] =spx.Close.resample('W', label='right').last().values
final_df.iloc[4,9] =spx.High.resample('W', label='right').max().values
final_df.iloc[5,9] =spx.Low.resample('W', label='right').min().values
...
final_df.iloc[1,1] =spy.Close.resample('W', label='right').last().values
#final_df.iloc[4,1] =spy.Volume.resample('W', label='right').last().values
...
final_df.iloc[5,5] =dji.Volume.resample('W', label='right').last().values
high_value = dji.High.resample('W', label='right').max().values[0]
final_df.iloc[11,5] =dji[dji.High == high_value].Volume.values
low_value = dji.Low.resample('W', label='right').min().values[0]
final_df.iloc[12,5] =dji[dji.Low == low_value].Volume.values[0]
...
final_df.iloc[6,7] =ixic.Close.resample('W', label='right').last().values
final_df.iloc[9,7] =ixic.High.resample('W', label='right').max().values
final_df.iloc[10,7] =ixic.Low.resample('W', label='right').min().values
high_value = ixic.High.resample('W', label='right').max().values[0]
final_df.iloc[12,7] =ixic[ixic.High == high_value].Volume.values[0]
low_value = ixic.Low.resample('W', label='right').min().values[0]
final_df.iloc[13,7] =ixic[ixic.Low == low_value].Volume.values[0]
...
#final_df.iloc[5,1] =spy.Volume.resample('W', label='right').last().values
...
final_df.iloc[1,5] =dji.Close.resample('W', label='right').last().values
final_df.iloc[6,5] =dji.Volume.resample('W', label='right').last().values
'''final_df.iloc[7,5] =dji.High.resample('W', label='right').max().values
final_df.iloc[8,5] =dji.Low.resample('W', label='right').min().values
high_value = dji.High.resample('W', label='right').max().values[0]
final_df.iloc[10,5] =dji[dji.High == high_value].Volume.values[0]
low_value = dji.Low.resample('W', label='right').min().values[0]
final_df.iloc[11,5] =dji[dji.Low == low_value].Volume.values[0]
'''
...
final_df.iloc[11,7] =ixic.Close.resample('W', label='right').last().values

spx= web.DataReader('^GSPC', 'yahoo',end - dt.timedelta(days=20),end - dt.timedelta(days=14))
final_df.iloc[2,9] =spx.Close.resample('W', label='right').last().values
...
high_value = spx.High.resample('W', label='right').max().values[0]
final_df.iloc[15,9] =spx[spx.High == high_value].Volume.values[0]
low_value = spx.Low.resample('W', label='right').min().values[0]
final_df.iloc[16,9] =spx[spx.Low == low_value].Volume.values[0]

hsi= web.DataReader('^HSI', 'yahoo',end - dt.timedelta(days=20),end - dt.timedelta(days=14))
final_df.iloc[3,11] =hsi.Close.resample('W', label='right').last().values
...
final_df.iloc[12,5] =dji.Volume.resample('W', label='right').last().values
...
final_df.iloc[3,9] =spx.Close.resample('W', label='right').last().values
