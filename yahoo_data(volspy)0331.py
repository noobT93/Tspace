...
#主表
final_df = pd.read_excel(r'C:/Users/Tao/Desktop/投资/spy/vol下单模板0331.xlsx',sheet_name = 1)
...
try:
    ...
    final_df.iloc[3,13] = sh_df1[sh_df1.index == (end - dt.timedelta(days=5))].最低.values
except:
...
#上证T-2周
try:
    final_df.iloc[4,13] = sh_df1[sh_df1.index == (end - dt.timedelta(days=12))].最低.values
    ...
    final_df.iloc[6, 13] = sh_df1[sh_df1.index == (end - dt.timedelta(days=19))].收盘.values
...
try:
    ...
    final_df.iloc[5, 13] = sh_df1[sh_df1.index == (end - dt.timedelta(days=19))].最低.values
    ...
    final_df.iloc[5, 13] = sh_df1[sh_df1.index == (end - dt.timedelta(days=26))].最低.values
...
final_df.iloc[3,9] =spx.High.resample('W', label='right').max().values
