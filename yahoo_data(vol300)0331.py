...
#截止时间（下单日前周六）
end = dt.datetime(2020,6,6)
#检查时把取数中A股隐藏避免取数错误

#主表
final_df = pd.read_excel(r'C:/Users/Tao/Desktop/投资/HS300/vol300下单模板0331.xlsx',sheet_name = 1)
...
try:
    ...
    final_df.iloc[1,1] = etf_df1[etf_df1.index == (end + dt.timedelta(days=1))].最高.values
    final_df.iloc[4,1] = etf_df1[etf_df1.index == (end + dt.timedelta(days=1))].最低.values
    ...
    final_df.iloc[1,1] = etf_df1[etf_df1.index == (end + dt.timedelta(days=6))].最高.values
    final_df.iloc[4,1] = etf_df1[etf_df1.index == (end + dt.timedelta(days=6))].最低.values
    range_df = etf_df.loc[[datei for datei in etf_df.index if etf_df1.index[-3] < datei < etf_df1.index[-1]], :]

final_df.iloc[8, 1] = range_df[range_df.收盘== range_df.收盘[-1]]['总手(万)'].values

#etf T-1周
try:
    final_df.iloc[2,1] = etf_df1[etf_df1.index == (end - dt.timedelta(days=6))].最高.values
    final_df.iloc[5,1] = etf_df1[etf_df1.index == (end - dt.timedelta(days=6))].最低.values
    final_df.iloc[6,1] = etf_df1[etf_df1.index == (end - dt.timedelta(days=6))].收盘.values
    ...
    final_df.iloc[5,1] = etf_df1[etf_df1.index == (end - dt.timedelta(days=13))].最低.values
    final_df.iloc[6,1] = etf_df1[etf_df1.index == (end - dt.timedelta(days=13))].收盘.values
    range_df = etf_df.loc[[datei for datei in etf_df.index if etf_df1.index[-4] < datei < etf_df1.index[-2]], :]

final_df.iloc[9, 1] = range_df[range_df.最高== max(range_df.最高)]['总手(万)'].values

#etf T-2周
try:
    final_df.iloc[3,1] = etf_df1[etf_df1.index == (end - dt.timedelta(days=13))].最高.values
    final_df.iloc[7,1] = etf_df1[etf_df1.index == (end - dt.timedelta(days=13))].收盘.values
    range_df = etf_df.loc[[datei for datei in etf_df.index if etf_df1.index[-4] < datei < etf_df1.index[-3]], :]
except:
    final_df.iloc[3,1] = etf_df1[etf_df1.index == (end - dt.timedelta(days=20))].最高.values
    final_df.iloc[7,1] = etf_df1[etf_df1.index == (end - dt.timedelta(days=20))].收盘.values
    range_df = etf_df.loc[[datei for datei in etf_df.index if etf_df1.index[-5] < datei < etf_df1.index[-3]], :]

final_df.iloc[11, 1] = range_df[range_df.最低== min(range_df.最低)]['总手(万)'].values
final_df.iloc[13, 1] = range_df[range_df.收盘== range_df.收盘[-1]]['总手(万)'].values
...
final_df.iloc[10, 1] = range_df[range_df.最高== max(range_df.最高)]['总手(万)'].values
final_df.iloc[12, 1] = range_df[range_df.最低== min(range_df.最低)]['总手(万)'].values
final_df.iloc[14, 1] = range_df[range_df.收盘== range_df.收盘[-1]]['总手(万)'].values
...
#上证T-1周
try:
    final_df.iloc[1,3] = sh_df1[sh_df1.index == (end - dt.timedelta(days=6))].最高.values
    ...
    final_df.iloc[1,3] = sh_df1[sh_df1.index == (end - dt.timedelta(days=13))].最高.values
...
#final_df.iloc[2, 3] = range_df[range_df.最高== max(range_df.最高)]['总手(万)'].values
...
final_df.iloc[8, 3] = range_df[range_df.最低== min(range_df.最低)]['总手(万)'].values
final_df.iloc[10, 3] = range_df[range_df.收盘== range_df.收盘[-1]]['总手(万)'].values

#上证T-3周
try:
    final_df.iloc[3, 3] = sh_df1[sh_df1.index == (end - dt.timedelta(days=20))].最低.values
    final_df.iloc[4, 3] = sh_df1[sh_df1.index == (end - dt.timedelta(days=20))].收盘.values
    ...
    final_df.iloc[4, 3] = sh_df1[sh_df1.index == (end - dt.timedelta(days=27))].收盘.values
...
try:
    ...
    final_df.iloc[5,5] = sz_df1[sz_df1.index == (end - dt.timedelta(days=6))].收盘.values
    ...
    final_df.iloc[5,5] = sz_df1[sz_df1.index == (end - dt.timedelta(days=13))].收盘.values
...
#深证T-2周
try:
    final_df.iloc[3, 5] = sz_df1[sz_df1.index == (end - dt.timedelta(days=13))].最高.values
    final_df.iloc[6, 5] = sz_df1[sz_df1.index == (end - dt.timedelta(days=13))].收盘.values
    ...
    final_df.iloc[6, 5] = sz_df1[sz_df1.index == (end - dt.timedelta(days=20))].收盘.values
    range_df = sz_df.loc[[datei for datei in sz_df.index if sz_df1.index[-5]<datei<sz_df1.index[-3]],:]

final_df.iloc[8, 5] = range_df[range_df.最高== max(range_df.最高)]['总手(万)'].values
final_df.iloc[10, 5] = range_df[range_df.最低== min(range_df.最低)]['总手(万)'].values
final_df.iloc[11, 5] = range_df[range_df.收盘== range_df.收盘[-1]]['总手(万)'].values
...
final_df.iloc[9,5] = range_df[range_df.最高== max(range_df.最高)]['总手(万)'].values
final_df.iloc[12,5] = range_df[range_df.收盘== range_df.收盘[-1]]['总手(万)'].values

#T周
hsi= web.DataReader('^HSI', 'yahoo',end - dt.timedelta(days=6),end )
final_df.iloc[0,7] =hsi.Close.resample('W', label='right').last().values
final_df.iloc[6,7] =hsi.Volume.resample('W', label='right').last().values
...
final_df.iloc[1,11] =ixic.High.resample('W', label='right').max().values
...
final_df.iloc[1,13] =spx.High.resample('W', label='right').max().values
...
final_df.iloc[2,7] =hsi.Low.resample('W', label='right').min().values
final_df.iloc[8,7] =hsi.Volume.resample('W', label='right').last().values
'''high_value = hsi.High.resample('W', label='right').max().values[0]
final_df.iloc[1,7] =hsi[hsi.High == high_value].Volume.values'''

dji= web.DataReader('^DJI', 'yahoo',end - dt.timedelta(days=13),end - dt.timedelta(days=7))
final_df.iloc[2,9] =dji.High.resample('W', label='right').max().values
final_df.iloc[4,9] =dji.Low.resample('W', label='right').min().values
high_value = dji.High.resample('W', label='right').max().values[0]
final_df.iloc[10,9] =dji[dji.High == high_value].Volume.values

ixic= web.DataReader('^IXIC', 'yahoo',end - dt.timedelta(days=13),end - dt.timedelta(days=7))
final_df.iloc[2,11] =ixic.High.resample('W', label='right').max().values
high_value = ixic.High.resample('W', label='right').max().values[0]
final_df.iloc[6,11] =ixic[ixic.High == high_value].Volume.values

spx= web.DataReader('^GSPC', 'yahoo',end - dt.timedelta(days=13),end - dt.timedelta(days=7))
final_df.iloc[3,13] =spx.Low.resample('W', label='right').min().values
high_value = spx.High.resample('W', label='right').max().values[0]
final_df.iloc[6,13] =spx[spx.High == high_value].Volume.values

#T-2周
hsi= web.DataReader('^HSI', 'yahoo',end - dt.timedelta(days=20),end - dt.timedelta(days=14))
final_df.iloc[1,7] =hsi.High.resample('W', label='right').max().values
final_df.iloc[3,7] =hsi.Low.resample('W', label='right').min().values
final_df.iloc[5,7] =hsi.Close.resample('W', label='right').last().values
'''high_value = hsi.High.resample('W', label='right').max().values[0]
final_df.iloc[1,7] =hsi[hsi.High == high_value].Volume.values'''

dji= web.DataReader('^DJI', 'yahoo',end - dt.timedelta(days=20),end - dt.timedelta(days=14))
final_df.iloc[3,9] =dji.High.resample('W', label='right').max().values
final_df.iloc[5,9] =dji.Low.resample('W', label='right').min().values
final_df.iloc[7,9] =dji.Close.resample('W', label='right').last().values
high_value = dji.High.resample('W', label='right').max().values[0]
final_df.iloc[11,9] =dji[dji.High == high_value].Volume.values
final_df.iloc[12,9] =dji.Volume.resample('W', label='right').last().values

ixic= web.DataReader('^IXIC', 'yahoo',end - dt.timedelta(days=20),end - dt.timedelta(days=14))
final_df.iloc[3,11] =ixic.High.resample('W', label='right').max().values

spx= web.DataReader('^GSPC', 'yahoo',end - dt.timedelta(days=20),end - dt.timedelta(days=14))
final_df.iloc[2,13] =spx.High.resample('W', label='right').max().values
low_value = spx.Low.resample('W', label='right').min().values[0]
final_df.iloc[7,13] =spx[spx.Low == low_value].Volume.values
final_df.iloc[9,13] =spx.Volume.resample('W', label='right').last().values

#T-3周
hsi= web.DataReader('^HSI', 'yahoo',end - dt.timedelta(days=27),end - dt.timedelta(days=21))
final_df.iloc[4,7] =hsi.Low.resample('W', label='right').min().values
high_value = hsi.High.resample('W', label='right').max().values[0]
final_df.iloc[7,7] =hsi[hsi.High == high_value].Volume.values

dji= web.DataReader('^DJI', 'yahoo',end - dt.timedelta(days=27),end - dt.timedelta(days=21))
final_df.iloc[6,9] =dji.Low.resample('W', label='right').min().values
final_df.iloc[8,9] =dji.Close.resample('W', label='right').last().values
#high_value = dji.High.resample('W', label='right').max().values[0]
#final_df.iloc[4,9] =dji[dji.High == high_value].Volume.values

ixic= web.DataReader('^IXIC', 'yahoo',end - dt.timedelta(days=27),end - dt.timedelta(days=21))
final_df.iloc[4,11] =ixic.Close.resample('W', label='right').last().values
high_value = ixic.High.resample('W', label='right').max().values[0]
final_df.iloc[7,11] =ixic[ixic.High == high_value].Volume.values

spx= web.DataReader('^GSPC', 'yahoo',end - dt.timedelta(days=27),end - dt.timedelta(days=21))
final_df.iloc[4,13] = spx.Close.resample('W', label='right').last().values
low_value = spx['Low'].resample('W', label='right').min().values[0]
final_df.iloc[8,13] =spx[spx.Low == low_value].Volume.values

final_df = final_df.applymap(lambda y: np.nan if y == 0 else y )

final_df.to_excel(r'C:/Users/Tao/Desktop/投资/HS300/vol取数.xlsx' , index=True, encoding='utf_8_sig')